...
int main(int argc, char *argv[]) {
    ...
    if (known_id >= 0) {
        for (i = 0; i < default_num_contexts; i++) {
            results[i].err = 0;
            if ((results[i].execs & (1 << 0)) && (results[i].crclist != list_known_crc[known_id])) {
            ...
            if ((results[i].execs & (1 << 1)) && (results[i].crcmatrix != matrix_known_crc[known_id])) {
            ...
            if ((results[i].execs & (1 << 2)) && (results[i].crcstate != state_known_crc[known_id])) {
    ...
    printf("Compiler version : %s\n",
           "GCC"
           "Clang 16.0.0 (https://github.com/llvm/llvm-project.git f81f0cb75a2808a67d2662f044ad07628fc9d900)");
